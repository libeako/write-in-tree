...
import qualified WriteInTree.Output.Pagination as OData
...
render_paragraph is_page_break sentencing p site = 
	let
		...
		content = 
			if not sentencing 
				then map (snd >>> flip render_inline site) p
				else
					let
						all_sections :: [[OData.Inline OData.AO UI.NodeIdU]]
						all_sections = (map snd >>> unite_neighboring_texts >>> Sentence.sentences) p
						render_possibly_sentence' il = 
							Xml.element_as_content (Html.classify_into [text_class_sentence] (map (flip render_inline site) il))
