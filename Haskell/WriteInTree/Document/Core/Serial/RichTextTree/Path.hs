...
type PictureElemO e = Ord.Ordered (Tt.Elem e)
...
picture_oi (ord, elem) = (PositionAtLevel ord (Tt.elemValue elem), elem)

picture_io :: PictureElemI e -> PictureElemO e
picture_io (PositionAtLevel ord _, elem) = (ord, elem)
...
type DataElemO e = ([PositionAtLevel], Ord.Ordered (Tt.Elem e))
...
data_io (path, (pos, elem)) = (map fst path, (ordinal pos, elem))

data_oi_2 :: Ord.Ordered Tt.Elem' -> PictureElemIT
data_oi_2 (ord, elem) = (PositionAtLevel ord (Tt.elemValue elem), elem)
